...
stiffness _k_slip_ in series with a Coulomb slider of slip-force limit _f_lim_. This is the
...
- _Stuck_ regime: `|k_slip * (q - z)| < f_lim` → force = `k_slip * (q - z)`, slider `z`
  unchanged.
- _Sliding_ regime: `|k_slip * (q - z)| >= f_lim` → force = `±f_lim`, slider `z` updates to
  `q ∓ f_lim / k_slip`.
