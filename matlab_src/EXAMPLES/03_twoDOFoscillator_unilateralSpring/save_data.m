% save_data.m — Full HB + Shooting + Floquet stability for twoDOFoscillator_unilateralSpring.
...
% Amplitude: fundamental harmonic magnitude of DOF 2 (index n+2 and 2n+2)
...
ds  = .02;
Sopt2 = struct('Dscale',[1e0*ones(size(ys));Om_s],'dynamicDscale',1);
Np = 1;
...
for i = 1:length(Om_shoot)
    ...
    if i > 1 && stable(i) ~= stable(i-1)
        ...
        BPs(nBP).a_rms = (a_shoot(i)+a_shoot(i-1))/2;
...
%% Save data
% a_rms using all harmonics of DOF 0 (odd rows = DOF 0 entries)
...
hold on;
plot(Om_HB, a_HB, 'g-', 'linewidth', 1.5);
plot(Om_shoot, a_shoot, 'k--', 'markersize', 5);
...
legend('HB','Shooting','unstable','location','northwest');
xlim(sort([Om_s Om_e]));
hold off;
box on; grid on;
